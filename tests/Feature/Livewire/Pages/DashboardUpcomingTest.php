        ...
        ->has(
            Season::factory()
                ->has(Episode::factory(rand(10, 30), [
                    'air_date' => fn () => Carbon::now()->addDays(rand(0, 365))
                ]))
        )
        ->count(5)
        ->create();

    $episodes = Episode::all()->sortBy('air_date');
...
it("groups air dates together", function () {
    ...
    $episodes = Episode::all()
        ...
        ->groupBy(function($item) {
            $diff = Carbon::now()->diffInDays($item->air_date);
            if($diff < 7) {
    ...
    Livewire::test(DashboardUpcoming::class)
        ->assertSeeInOrder($episodes->map(function($items, $key) {
            if(Str::startsWith($key, 'd')) {
                return $items->first()->air_date->format('l');
            }else if(Str::startsWith($key, 'w')) {
                return $items->first()->air_date->startOfWeek()->format('M jS');
            }else{
                return $items->first()->air_date->startOfMonth()->format('M');
            }
        })->toArray());
});
