...
$textClasses = Flux::classes()
    ->add('text-2xl font-serif font-medium truncate [:where(&)]:text-zinc-800 dark:[:where(&)]:text-zinc-100')
    ;
@endphp
...
<?php if ($name): ?>
    <a href="{{ $href }}" {{ $attributes->class([ $classes, 'gap-2' ]) }} data-flux-brand>
        ...
        <?php if ($name instanceof \Illuminate\View\ComponentSlot): ?>
            <div {{ $name->attributes->class($textClasses) }}>{{ $name }}</div>
        <?php else: ?>
